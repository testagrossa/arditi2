...
trait EncoderInstances:
  ...
  given booleanEncoder: Encoder[Boolean] =
    Encoder.fromFunction(b => Json.Bool(b))// TODO Define a given instance of type `Encoder[Boolean]`
...
trait DecoderInstances:
  ...
  given listDecoder[A](using decoder: Decoder[A]): Decoder[List[A]] =
    ...
    def decodeAndPrepend(item: Json, previouslyDecodedItems: List[A]): Option[List[A]] =
      /*
      decoder.decode(item).map { a =>
        a :: previouslyDecodedItems
      }
      */
      for { a <- decoder.decode(item) } yield a :: previouslyDecodedItems
    ...
    def processItem(item: Json, maybePreviouslyDecodedItems: Option[List[A]]): Option[List[A]] =
      /*
      maybePreviouslyDecodedItems.flatMap { previouslyDecodedItems =>
        decodeAndPrepend(item, previouslyDecodedItems)
      }
      */
      for {
        previouslyDecodedItems <- maybePreviouslyDecodedItems
        maybeDecoded <- decodeAndPrepend(item, previouslyDecodedItems)
      } yield maybeDecoded
    ...
    Decoder.fromFunction {
      case Json.Arr(items) =>
        items.foldRight(Option.apply(List.empty[A])) { case (item, maybeListA) =>
          /*
          maybeListA.flatMap { listA =>
            decoder.decode(item).map { a =>
              a :: listA
            }
          }
          */
          for {
            listA <- maybeListA
            a <- decoder.decode(item)
          } yield a :: listA
        }
      case json => decoder.decode(json).map { a => List(a) }
    }
  ...
  def field[A](name: String)(using decoder: Decoder[A]): Decoder[A] =
    Decoder.fromPartialFunction {
      case Json.Obj(fields: Map[String, Json]) if fields.contains(name) && decoder.decode(fields(name)).isDefined =>
